<?php

namespace App\Http\Controllers\V2\Admin;

use App\Http\Controllers\Controller;
use App\Models\Category;
use App\Models\Fund;
use App\Models\Unit;
use App\Traits\UnitTrait;
use Hashids\Hashids;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\Config;

class CategoryController extends Controller
{
    use UnitTrait;

    public function create($unitHashId)
    {
        $unitIDHashID = new Hashids('unit id hash',10,Config::get('app.encode_chars'));
        $unitId = $unitIDHashID->decode($unitHashId);

        $unit = Unit::findOrFail($unitId[0]);
        $availableFunds =Fund::getUnitDonatedFund($unitId[0]);
        $awardedFunds =Fund::getUnitAwardedFund($unitId[0]);
        $categories = Category::query()
            ->where('unit_id', $unitId[0])
            ->get();

        $issueResolutions = $this->calculateIssueResolution($unitId[0]);

        view()->share('totalIssueResolutions',$issueResolutions);
        view()->share('availableFunds',$availableFunds );
        view()->share('awardedFunds',$awardedFunds );
        view()->share('unitObj',$unit);
        view()->share('categories',$categories);
        return view('admin.settings.create');
    }
    public function store(Request $request)
    {
        Category::create([
            'unit_id'         => $request->unit_id,
            'title'           => $request->title,
            'status'          => $request->status
        ]);

        $unit = Unit::findOrFail($request->unit_id);
        $availableFunds =Fund::getUnitDonatedFund($request->unit_id);
        $awardedFunds =Fund::getUnitAwardedFund($request->unit_id);
        $categories = Category::query()
            ->where('unit_id', $request->unit_id)
            ->get();

        $issueResolutions = $this->calculateIssueResolution($request->unit_id);

        view()->share('totalIssueResolutions',$issueResolutions);
        view()->share('availableFunds',$availableFunds );
        view()->share('awardedFunds',$awardedFunds );
        view()->share('unitObj',$unit);
        view()->share('categories',$categories);
        return view('admin.settings.index');
    }

    public function edit($categoryId, $unitId)
    {
        $unitIDHashID = new Hashids('unit id hash',10,Config::get('app.encode_chars'));
        $unitId = $unitIDHashID->decode($unitId);

        $unit = Unit::findOrFail($unitId[0]);
        $availableFunds =Fund::getUnitDonatedFund($unitId[0]);
        $awardedFunds =Fund::getUnitAwardedFund($unitId[0]);
        $category = Category::query()
            ->where('unit_id', $unitId[0])
            ->where('id', $categoryId)
            ->first();

        $issueResolutions = $this->calculateIssueResolution($unitId[0]);

        view()->share('totalIssueResolutions',$issueResolutions);
        view()->share('availableFunds',$availableFunds );
        view()->share('awardedFunds',$awardedFunds );
        view()->share('unitObj',$unit);
        view()->share('category',$category);
        return view('admin.settings.edit');
    }

    public function update(Request $request, $categoryId)
    {
        Category::where('id', $categoryId)
            ->where('unit_id', $request->unit_id)
            ->update([
            'title'           => $request->title,
            'status'          => $request->status
        ]);

        $unit = Unit::findOrFail($request->unit_id);
        $availableFunds =Fund::getUnitDonatedFund($request->unit_id);
        $awardedFunds =Fund::getUnitAwardedFund($request->unit_id);
        $categories = Category::query()
            ->where('unit_id', $request->unit_id)
            ->get();

        $issueResolutions = $this->calculateIssueResolution($request->unit_id);

        view()->share('totalIssueResolutions',$issueResolutions);
        view()->share('availableFunds',$availableFunds );
        view()->share('awardedFunds',$awardedFunds );
        view()->share('unitObj',$unit);
        view()->share('categories',$categories);
        return view('admin.settings.index');
    }
}

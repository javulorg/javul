    ...
    public function home($unit_id = '',$slug = '')
    {
        ...
        if(!empty($unit_id))
        {
            ...
            if(!empty($unit))
            {
                ...
                $homeCheck = 'false';
//                dd($pages['pages'][0]);
                return view("wiki.home", compact('homeCheck'));
            }
        }
...
//                dd($pages);
        ...
        if(!empty($unit_id))
        {
            ...
            if(!empty($unit))
            {
                ...
                if(!empty($pages['pages'])){
                    ...
                    $changes = Wiki::getChangesFor($filter);
    ...
    public function changes($unit_id , $slug, $wiki_page_id)
    {
        view()->share("slug",$slug);
        view()->share("unit_id",$unit_id);
        $unitIDHashID = new Hashids('unit id hash',10,Config::get('app.encode_chars'));
    ...
    public function edit( Request $request ,$unit_id , $slug , $wiki_page_id = 0)
    {
        ...
        if(!empty($unit_id))
        {
            ...
            if($request->isMethod('post'))
            {
                ...
                if((int)$inputData['wiki_page_rev_id'] == 0 && $inputData['is_wikihome'] = 0 )
                {

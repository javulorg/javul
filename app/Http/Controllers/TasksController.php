...
class TasksController extends Controller
{
    ...
    public function getObjectiveTask($objectiveId, $slug, $unitId)
    {
        ...
        return view('tasks.create', ['totalIssueResolutions' => '1']);
    }
        ...
        if (!empty($task_id)) {
            ...
            if (!empty($task_id)) {
                ...
                if (!empty($taskObj) && count($taskEditor) > 0) {
                    ...
                    TaskEditor::where('task_id', $task_id)->where('user_id', Auth::user()->id)->update([
            ...
            if (!empty($task_id)) {
                ...
                if (!empty($taskObj)) {
                    if ($request->isMethod('post')) {
                        ...
                        $taskBidder = TaskBidder::where('task_id', $task_id)->where('user_id', Auth::user()->id)->where(
                            'task_bidders.status',
                            'offer_accepted'
                        )->first();
                        if (!empty($taskBidder))

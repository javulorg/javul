...
class HomeController extends Controller
{
    public function __construct()
    {
        $this->middleware('auth', ['except' => [
            'index',
            'add_to_watchlist',
            'get_unit_site_activity_paginate',
            'get_site_activity_paginate',
            'global_activities',
            'get_categories',
            'browse_categories',
            'get_next_level_categories',
            'global_search',
            'check_username',
            'check_email',
            'skill_view'
            ...
            if ($type == "old") {
                ...
                if (count($unitCategoryObj) > 0 && !empty($unitCategoryObj)) {
                    ...
                    return response()->json([
                        'success' => true,
                        'category_id' => $unitCategoryObj->id,
                        'type' => $type,
        ...
        if (Auth::user()->role == "superadmin") {
            ...
            if ($type == "old") {
                ...
                if (count($areaOfInterestObj) > 0 && !empty($areaOfInterestObj)) {
                    ...
                    return response()->json([
                        'success' => true,
                        'area_of_interest_id' => $areaOfInterestObj->id,
                        'type' => $type,
                        'title' => $data['title']
                    ]);
                }
                ...
                if (!empty($prefix_id)) {
                    ...
                    if (!empty($unitCategoryHistory) && count($unitCategoryHistory) > 0) {
                        ...
                        if ($unitCategoryHistory->action_type == "add") {
                            ...
                            $html = '<a href="' . url('userprofiles/' . $user_id . '/' . strtolower(Auth::user()->first_name . '_' . Auth::user()->last_name)) . '">'
                                . $loggedinUsername . '</a> approved addition of category <a href="' . url('site_admin') . '">';
                            ...
                            $html = '<a href="' . url('userprofiles/' . $user_id . '/' . strtolower(Auth::user()->first_name . '_' . Auth::user()->last_name)) . '">'
                                . $loggedinUsername . '</a> approved deletion of category <a href="' . url('site_admin') . '">';
                ...
                if (Auth::user()->role == "superadmin") {
                    ...
                    if (!empty($prefix_id)) {
                        ...
                        if (!empty($jobSkillHistory) && count($jobSkillHistory) > 0) {
                            ...
                            $html = '<a href="' . url('userprofiles/' . $user_id . '/' . strtolower(Auth::user()->first_name . '_' . Auth::user()->last_name)) . '">'
                                . $loggedinUsername . '</a> rejected ' . $op_type . ' of skill <a href="' . url('site_admin') . '">';
                ...
                if (Auth::user()->role == "superadmin") {
                    ...
                    if (!empty($prefix_id)) {
                        ...
                        if (!empty($unitCategoryHistory) && count($unitCategoryHistory) > 0) {
                            ...
                            $html = '<a href="' . url('userprofiles/' . $user_id . '/' . strtolower(Auth::user()->first_name . '_' . Auth::user()->last_name)) . '">'
                                . $loggedinUsername . '</a> rejected ' . $op_type . ' of category <a href="' . url('site_admin') . '">';
    ...
    public function browse_skills(Request $request)
    {
        if ($request->ajax()) {
            if (Auth::check()) {
                ...
                view()->share('firstBox_skills', $firstBox_skills);
                $selected_skills = [];
                $job_skill_list = JobSkill::pluck('skill_name', 'id')->all();

...
class UnitsController extends Controller
{
        ...
        if (!empty($unit_id)) {
            ...
            if (!empty($unit_id)) {
                ...
                if (!empty($units) && $request->isMethod('post')) {
                    ...
                    view()->share('unitObj', $units);
                    view()->share('unitHashId', $unitHashId);
    ...
    public function update(Request $request, $unitHashId)
    {
        ...
        $bytes = UnitRevision::strBytes(str_replace(' ', '', strip_tags($request->input('description'))));
        $oldBytes = UnitRevision::strBytes(str_replace(' ', '', strip_tags($unit->description)));

        $UnitRevision = new UnitRevision();
        $UnitRevision->unit_id         = $unit->id;
        $UnitRevision->user_id         = $unit->user_id;
        $UnitRevision->category_id     = $unit->category_id;
        $UnitRevision->name            = $unit->name;
        $UnitRevision->description     = $unit->description;
        $UnitRevision->comment         = $request->comment;
        $UnitRevision->credibility     = $unit->credibility;
        $UnitRevision->country_id      = (int)$unit->country_id;
        $UnitRevision->state_id        = (int)$unit->state_id;
        $UnitRevision->city_id         = (int)$unit->city_id;
        $UnitRevision->status          = $unit->status;
        $UnitRevision->parent_id       = $unit->parent_id;
        $UnitRevision->modified_by     = Auth::user()->id;
        $UnitRevision->size            = ($bytes - $oldBytes);
        $UnitRevision->created_at      = date("Y-m-d H:i:s");
    ...
    public function view($unit_id, Request $request)
    {
        if (!empty($unit_id)) {
            $unitIDHashID = new Hashids('unit id hash', 10, Config::get('app.encode_chars'));
            $unit_id = $unitIDHashID->decode($unit_id);
            if (!empty($unit_id)) {
                ...
                if (!empty($unit)) {
                    ...
                    $taskBidders = [];
                    if (!empty($userAuth))
                        $taskBidders = Task::join('task_bidders', 'tasks.id', '=', 'task_bidders.task_id')->where(
                            'task_bidders.user_id',
                            Auth::user()->id
                        )->where('unit_id', $unit_id)->where('tasks.status', '=', "approval")->count();
                    ...
                    $issuesObj = Issue::where('unit_id', $unit_id)->orderBy('id', 'desc')
                        ->paginate(Config::get('app.page_limit'));
    ...
    public function removeU($unitId)
    {
        $userId = auth()->id();
        ...
        if ($watch) {
            ...
            return response()->json(['message' => 'Not found in watchlist.']);
        }
    }
}

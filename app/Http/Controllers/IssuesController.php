...
class IssuesController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('issues.index', compact('homeCheck', 'issuesTotal', 'pagination'));
    }
    ...
    public function pluck(Request $request)
    {
        ...
        if (!empty($unit_id)) {
            ...
            if (!empty($unit_id)) {
                ...
                if (!empty($unitObj)) {
                    view()->share('unitObj', $unitObj);
                    $issuesObj = Issue::leftJoin('importance_level', 'issues.id', '=', 'importance_level.issue_id')->where(
                        'unit_id',
                        $unit_id
                    )
                        ->orderBy('id', 'desc')
                        ->select(['issues.*'])
                        ->paginate(Config::get('app
                    .page_limit'));

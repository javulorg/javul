...
class Forum extends Model
{
    ...
    public static function getPost($filter,$paginate = false){
        ...
        $topics = DB::table("forum_post")
            ->select([
                "forum_ideapoint.value as ideapoint",
                "forum_updown.value as updown",
                "forum_post.*",
                "users.first_name",
                "users.last_name",
                ...
                DB::raw("COALESCE((SELECT sum(value) FROM forum_updown as tempfup WHERE tempfup.post_id = forum_post.post_id), 0) as updownpoint")
            ])
            ...
            ->where($extraWhere)
            ->orderBy("forum_post.post_id", $filter['orderBy'] ?? "ASC");
    ...
    public static function submit($data)
    {
        ...
        DB::table('forum_topic')->where('id', $topicId)->update([
            'topic_id' => $topicId
        ]);
    	return $topicId;
    }

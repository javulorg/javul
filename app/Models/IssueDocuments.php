            ...
            if (count($files) > 0) {
                ...
                foreach ($files as $index => $file) {
                    if (!empty($file)) {
                        $rules = [
                            'document' => 'required',
                            'extension' => 'required|in:doc,docx,pdf,txt,jpg,png,ppt,pptx,jpeg,doc,xls,xlsx'
                        ];
                        $fileData = [
                            'document' => $file,
                        ...
                        if (!$validator->fails()) {
                            if ($file->isValid()) {
                                $destinationPath = public_path('uploads/issue/' . $issue_id_encoded);
                                ...
                                $fileName = $issue_id_encoded . '_' . $totalAvailableDocs . '.' . $extension;

                                $file->move($destinationPath, $fileName);

                                // Insert record into the database
                                $path = 'uploads/issue/' . $issue_id_encoded . '/' . $fileName;
                                IssueDocuments::create([
                                    'issue_id' => $issue_id,
                                    'file_name' => $file_name,
                                    'file_path' => $path

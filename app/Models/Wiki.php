...
class Wiki extends Model
{
    ...
    public static function createNotExist($unitDetail,$filter = array()){
        ...
        if($check->count() == 0)
        {
            ...
            else if($filter['is_wikihome'] == 3)
            {
             ...
             $wikiId = DB::table('wiki_pages')->insertGetId($wiki_pages);
            DB::table('wiki_pages')->where('id', $wikiId)->update([
                'wiki_page_id'  => $wikiId
             ]);
             return $wikiId;
        }
    ...
    public static function updateData($data)
    {
        ...
        if((int)$data['wiki_page_rev_id'] > 0)
        {
            ...
            $wikiArchId = DB::table('wiki_arch_revisions')->insertGetId($wiki_arch_revisions);
            DB::table('wiki_arch_revisions')
                ->where('id', $wikiArchId)
                ->update([
                    'revision_id'   => $wikiArchId
                ]);
        }

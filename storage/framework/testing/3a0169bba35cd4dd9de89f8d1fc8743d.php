<?php $__env->startSection('title', 'Wiki Page Recent Changes'); ?>
                ...
                <div class="card-body table-responsive loading_content_hide">
                    <div class="table-responsive">
                        <table class="table">
                            ...
                            <tbody>
                            <?php $prevKey = 0; ?>
                            <?php $__currentLoopData = $userWikiRev; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $page): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <?php
                                $nextPageId = 0;
                                ?>
                                <tr>
                                    <td>
                                        <a href="<?php echo e(route('user_wiki_rev_diff',[$slug,$user_id_hash,$page->id,$nextPageId ])); ?>">Diff</a> |
                                        <a href="<?php echo e(route('user_wiki_history',[str_replace(' ', '_', strtolower($userObj->first_name.' '.$userObj->last_name)), $user_id_hash, $userPageIDHashID->encode($page->page_id)])); ?>">Hist</a>
                                    </td>
                                    <td><a href="<?php echo e(route('user_wiki_view',[$slug, $userPageIDHashID->encode($page->id),$page->slug])); ?>"><?php echo e($page->page_title); ?></a></td>
                                    <td><?php echo e($Carbon::createFromFormat('Y-m-d H:i:s', $page->updated_at)->diffForHumans()); ?></td>
                                    <td><?php echo e($page->size > 0 ? '+'.$page->size : '-'.$page->size); ?></td>
                                    <td>
                                        <a href="<?php echo e(url('userprofiles/'. $userIDHashID->encode($page->user_id) .'/'.strtolower($page->first_name."_".$page->last_name))); ?>"><?php echo e($page->first_name . ' '. $page->last_name); ?></a>
                                        (<?php echo e($page->comment); ?>)
                                    </td>
                                </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </tbody>
                            <tbody>
                            <tr>
                                <td colspan="3"><?php echo e($userWikiRev->links()); ?></td>
                            </tr>
                            </tbody>
                        </table>
                    </div>
...
<?php echo $__env->make('layout.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /var/www/html/javul-staging/javul/resources/views/users/wiki/wiki_page_recentchange.blade.php ENDPATH**/ ?>
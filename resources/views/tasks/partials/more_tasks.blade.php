            ...
            <tr>
                ...
                <td width="20%" class="text-center">
                    @if($task->status == "editable")
                        <span class="colorLightGreen">{{\App\Models\SiteConfigs::task_status($task->status)}}</span>
                    @else
                        <span class="colorLightGreen">{{\App\Models\SiteConfigs::task_status($task->status)}}</span>
                    @endif
                </td>
                <td class="text-center">{{\App\Models\Task::getTaskCount('in-progress',$task->id)}}</td>
                <td class="text-center">{{\App\Models\Task::getTaskCount('completed',$task->id)}}</td>
            </tr>
        @else

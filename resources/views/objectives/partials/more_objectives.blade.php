    ...
    @foreach($objectives as $objective)
        @if($from_page == "unit_view")
            <tr>
                ...
                <td  class="text-center">{{\App\Models\Task::getTaskCount('available',$objective->id)}}</td>
                <td  class="text-center">{{\App\Models\Task::getTaskCount('in-progress',$objective->id)}}</td>
                <td  class="text-center">{{\App\Models\Task::getTaskCount('completed',$objective->id)}}</td>
            </tr>
        @else
            <tr>
                <td><a href="{!! url('objectives/'.$objectiveIDHashID->encode($objective->id).'/'.$objective->slug)!!}">{{$objective->name}}</a></td>
                <td><a href="{!! url('units/'.$unitIDHashID->encode($objective->unit_id).'/'.\App\Models\Unit::getSlug($objective->unit_id) )!!}">{{\App\Unit::getUnitName($objective->unit_id)}}</a></td>
                <td><a href="{!! url('userprofiles/'.$userIDHashID->encode($objective->user_id).'/'.strtolower
                                (\App\Models\User::getUserName($objective->user_id)))!!}">
                        {{\App\Models\User::getUserName($objective->user_id)}}
                    </a></td>
